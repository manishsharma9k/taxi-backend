...
const getRouteData = async (pickup, dropoff) => {
  ...
  try {
    ...
    try {
      const osrmRes = await fetch(`http://router.project-osrm.org/route/v1/driving/${lon1},${lat1};${lon2},${lat2}?overview=false`);
    ...
    return {
      ...
      durationMin: Math.ceil(Math.max(parseFloat((straight * 1.3).toFixed(1)), 1.0) * 3),
    };
  } catch (error) {
...
export const getEstimate = async (req, res) => {
  ...
  res.json({
    ...
    currency: '₹',
    options: [
      { id: 'bike', type: 'TaxiNova Bike', description: 'Beat the traffic',    price: bikeFare, eta: `${Math.max(Math.ceil(durationMin * 0.7), 2)} mins`, iconUrl: '🏍️' },
      { id: 'auto', type: 'TaxiNova Auto', description: 'Haggle-free rides',   price: autoFare, eta: `${Math.max(Math.ceil(durationMin * 0.9), 4)} mins`, iconUrl: '🛺' },
      { id: 'cab',  type: 'TaxiNova Cab',  description: 'Comfortable & clean', price: cabFare,  eta: `${Math.max(Math.ceil(durationMin * 1.1), 6)} mins`, iconUrl: '🚗' },
    ]
  });
};

export const bookRide = async (req, res) => {
  const { pickup, dropoff, fare, vehicleType, paymentMethod, pickupLat, pickupLng } = req.body;

  try {
    const user = req.user;
    if (!user) return res.status(401).json({ message: 'User not authorized' });

    const customRideId = `RID${Math.floor(100000 + Math.random() * 900000)}`;
    ...
    const newRide = await Ride.create({
      user: user._id,
      pickup, dropoff, fare: actualFare,
      vehicleType,
    ...
    const populatedRide = await Ride.findById(newRide._id).populate('user', 'name phone');
    ...
    if (pickupLat && pickupLng) {
      targetCaptains = nearbyCaptains.filter(c =>
        c.location?.lat && c.location?.lng &&
        getDistanceKm(pickupLat, pickupLng, c.location.lat, c.location.lng) <= 10
      );
    ...
    res.status(201).json({
      message: targetCaptains.length > 0
...
export const updateRideStatus = async (req, res) => {
  ...
  try {
    ...
    io.to(`user_${ride.user}`).emit('ride:statusUpdate', { rideId: ride._id, status });
    if (status === 'completed') {
      io.to(`captain_${ride.captain}`).emit('ride:completed', { rideId: ride._id });
    }
...
export const cancelRide = async (req, res) => {
  ...
  try {
    ...
    if (ride.captain) {
      io.to(`captain_${ride.captain}`).emit('ride:cancelled', { rideId: ride._id, reason: cancelReason });
    }
...
export const captainCancelRide = async (req, res) => {
  ...
  try {
    ...
    const isCaptainRide = ride.captain && ride.captain.toString() === req.captain._id.toString();
